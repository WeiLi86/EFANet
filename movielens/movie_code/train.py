...
from movie_code.autoint import AutoInt
from movie_code.InterHAt import InterHAt
from movie_code.fignn import FiGNN
from movie_code.fint import FINT
from movie_code.xcrossnet import XCrossNet
from movie_code.frnet import FRNet
from movie_code.adnfm import AdnFM
from movie_code.efanet import EFANet
import argparse
...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='FRANet')
...
def _run_(args, run_cnt):
    ...
    if args.model_type == 'AutoInt':
        ...
        model = FINT(args=args, feature_size=feature_size, run_cnt=run_cnt)
    elif args.model_type == 'XCrossNet':
        model = XCrossNet(args=args, feature_size=feature_size, run_cnt=run_cnt)
    elif args.model_type == 'FRNet':
        model = FRNet(args=args, feature_size=feature_size, run_cnt=run_cnt)
    elif args.model_type == 'AdnFM':
        model = AdnFM(args=args, feature_size=feature_size, run_cnt=run_cnt)
    elif args.model_type == 'EFANet':
        model = EFANet(args=args, feature_size=feature_size, run_cnt=run_cnt)
