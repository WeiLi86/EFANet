...
def fint_interaction_layer(values, field_size, lay_count, l2_reg):
    x0 = values
    xi = values
    for i in range(lay_count):
        xi = field_aware_interaction_layer(xi, x0, field_size, l2_reg)
    return xi
...
def field_aware_interaction_layer(xi, x0, field_num, l2_reg):
    ...
    context_vec = tf.layers.dense(x, field_num, use_bias=False,
                                  kernel_regularizer=tf.contrib.layers.l1_regularizer(l2_reg))
...
class FINT():
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            fint_output = fint_interaction_layer(self.embeddings, self.field_size, 3, 0.0)

            # dnn part
            self.y_dense = tf.reshape(fint_output, shape=[-1, self.field_size * self.embedding_size])
