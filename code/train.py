...
def parse_args():
    ...
    parser.add_argument('--has_residual', action='store_true', help='add residual', default=True)
    ...
    parser.add_argument('--deep_layers', type=str2list, default=[64])
...
def _run_(args, file_name, cnt):
    ...
    if args.model_type == 'AutoInt':
        ...
        model = EFANet(args=args, feature_size=feature_size, cnt=cnt)
    ...
    for k in range(model.epoch):
        ...
        for j in range(3, 11):
            ...
            t1 = time()
            is_continue = model.fit_once(Xi_train, Xv_train, y_train, k+1, file_count, Xi_valid, Xv_valid, y_valid, early_stopping=True)
            time_epoch += time() - t1
