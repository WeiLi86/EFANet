...
class XCrossNet():
    def __init__(self, args, feature_size):
        ...
        if args.data == 'criteo':
            self.sparse_feature_count = 26
            self.dense_feature_count = 13
        elif args.data == 'avazu':
            self.sparse_feature_count = 23
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            if self.dense_feature_count > 0:
                self.dense, self.sparse = self.embeddings[:, :self.dense_feature_count, :], self.embeddings[:,
                                                                                            self.dense_feature_count:,
                                                                                            :]
                self.dense = tf.reshape(self.dense, shape=[-1, self.embedding_size * self.dense_feature_count])
                self.dense = tf.layers.dense(self.dense, self.dense_feature_count, activation=tf.nn.sigmoid,
                                             use_bias=False)
            ...
            # inner product
            inner_product = tf.squeeze(innerProductLayer(self.sparse, sparse_feature=self.sparse_feature_count),
                                       axis=-1)
            ...
            self.out = tf.add(tf.matmul(self.y_dense, self.weights["prediction_dense"]),
    ...
    def _initialize_weights(self):
        ...
        for i in range(self.cross_num):
            weights["layer_%d" % i] = tf.Variable(
                np.random.normal(loc=0, scale=0.01, size=(self.dense_feature_count, 1)), dtype=np.float32)
